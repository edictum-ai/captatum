...
**Sibling (mirror patch, REDOS-8):** `selectMainContentHtml` (main-content.ts, the landmark path — hotter, every HTML page) had the SAME unbounded N×`extractVisibleText` shape on its `<article>`/`<main>` scoring. #165 introduces the cap pattern, so the mirror is applied in the same PR: the `<article>`/`<main>` candidate lists are sliced to `MAX_LANDMARK_CANDIDATES` (32) before scoring (document-order-preserving, so `firstArticle` is unchanged; real pages have 1–3). An attacker who learns the container path is capped can no longer pivot to an `<article>` flood. (This REDOS is body-budget-bound to ~2.5s at 5MB even uncapped, so a timing guard can't meaningfully distinguish capped/uncapped under the budget — covered by the cap + the existing selection tests, not a tautological timing test.)
