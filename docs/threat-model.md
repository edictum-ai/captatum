...
| Attack class | Bound |
| --- | --- |
| Directed DoS to a victim (count) | `maxPerHostInBulk` (10), **union-keyed on egress hosts** (seed registrable domain ∪ redirect hosts ∪ `finalUrl` ∪ Tier-2-resolved). Pre-egress: truncate each seed domain; post-egress: abort further seeds to a host over the cap (`bulk_per_host_cap`). Worst-case per-victim count is `maxPerHostInBulk + maxConcurrency - 1` (in-flight discovery overshoot — a victim host is added to the union only after a seed settles, so up to `maxConcurrency - 1` in-flight seeds can reach it before the cap aborts dispatch); rate-bounded independently by `maxPerHostInflight`. |
