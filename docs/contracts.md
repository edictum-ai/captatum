...
| `schema` | no | Uniform JSON schema for `output: extract`. Validated against the supported-keyword allowlist at the input boundary; an unsupported keyword is a whole-call `extract_schema_unsupported_keyword` reject before any seed is fetched (#153). |
...
`extract` validates the provider's JSON before returning it. The validator enforces the supported JSON Schema subset used by this tool (`type`, `required`, `properties`, `additionalProperties`, `items`, `enum`/`const`, string length/pattern, numeric bounds, array/property counts, uniqueness, and `allOf`/`anyOf`/`oneOf`/`not`). **Unsupported keywords are rejected at the input boundary** (`extract_schema_unsupported_keyword`, JSON-RPC `InvalidParams` / HTTP `{error:{code,message}}`, before any fetch/LLM) — the validator is an allowlist of supported keywords, and a keyword it cannot verify (e.g. `format`, `contentEncoding`, or a misplaced tool knob like `budget`) fails closed rather than accepting unvalidated structured data. The message leads with the offending key (`Unsupported JSON Schema keyword "budget" at $ — captatum cannot verify it; remove it.`) and never echoes a schema value (the schema is data, never a directive); the key name is length-capped. The same allowlist check runs for `captatum_bulk`'s uniform `schema` as a whole-call reject. A defense-in-depth copy of the check remains at the transform seam (`finalize`) for any path that bypasses input normalization; on the rare reach it degrades with `transform.reason: "schema_validation_failed"`. For **supported** keywords, a value mismatch (wrong type, `minLength`, etc.) is **advisory**: the parsed JSON is still returned (imperfect structured data > raw fallback) but the mismatch is surfaced as a non-fatal `extract_schema_invalid` error so the caller is not silently handed schema-violating data.
...
`too_many_urls` / `extract_schema_unsupported_keyword` for an unsupported extract-schema keyword), auth, admission `OverloadedError` (`-32050`),
