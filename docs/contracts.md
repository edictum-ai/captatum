...
- **Server request timeout** (`Fastify requestTimeout`, defense-in-depth beyond the per-tier `timeoutMs`): a whole-request wall so a hijacked/slow stream can't pin a connection. For `captatum_bulk` it is **derived from the bulk wall + a small margin** (`resolveRequestTimeout` → `BULK_GUARD_DEFAULTS.maxGlobalWallMs + 5 000` ms), so the HTTP backstop can never cut off the structured partial the wall assembles — and the two can never drift. Non-bulk requests use a fixed 90 s (#148).
