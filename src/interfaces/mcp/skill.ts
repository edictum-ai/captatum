...
function body(forLabel: string): string {
  return [
    ...
    "## Fetch",
    "```bash",
    "npx -y captatum <url>                    # summary (token-light; default with a provider)",
    "npx -y captatum <url> --output raw       # full clean content + structured data",
    'npx -y captatum <url> --output summary --prompt "..."  # summary answering a question',
    "npx -y captatum <url> --output extract --schema '{\"type\":\"object\",...}'  # fields as JSON",
    "npx -y captatum <url> --allow-render     # force a browser render (JS-only pages)",
    "npx -y captatum <url> --debug            # + diagnostics (tier, attempts, model/tokens)",
    "```",
    "",
    "## Token tips",
