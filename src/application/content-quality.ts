 ...
 *  an anti-bot challenge (captcha OR bot_verification), or a 4xx/5xx HTTP error is already a failure,
 *  not "low-quality content". */
export function classifyContentQuality(result: Result): ContentQuality | undefined {
  if (result.tier === "error" || result.challengeProvider || result.botVerification || Number(result.code) >= 400) return undefined;
