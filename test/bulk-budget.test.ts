...
test("BudgetTracker: beforeSeed reserves bytes; dispatch false when egress cap full", () => {
  ...
  const t = new BudgetTracker({ clock, maxGlobalEgressBytes: 100, maxGlobalWallMs: 5000, maxTransformCostUsd: 1, perSeedTransformCostUsd: 0.02, perSeedMaxBytes: 30, renderEgressUnits: 8 });
...
test("BudgetTracker: cost fail-soft — runTransform false when cost budget exhausted (fetch still happens)", () => {
  const clock = fakeClock();
  const t = new BudgetTracker({ clock, maxGlobalEgressBytes: 1_000_000, maxGlobalWallMs: 5000, maxTransformCostUsd: 0.10, perSeedTransformCostUsd: 0.02, perSeedMaxBytes: 100, renderEgressUnits: 8 });
...
test("BudgetTracker: reservation invariant — total fetched never exceeds the egress cap", () => {
  ...
  const clock = fakeClock();
  const t = new BudgetTracker({ clock, maxGlobalEgressBytes: 100, maxGlobalWallMs: 5000, maxTransformCostUsd: 1, perSeedTransformCostUsd: 0.01, perSeedMaxBytes: 30, renderEgressUnits: 8 });
  let dispatched = 0;
  for (let i = 0; i < 4; i++) {
    const b = t.beforeSeed();
    if (!b.dispatch) break;
...
test("BudgetTracker: wallExceeded flips after the deadline", () => {
  ...
  const t = new BudgetTracker({ clock, maxGlobalEgressBytes: 1000, maxGlobalWallMs: 5000, maxTransformCostUsd: 1, perSeedTransformCostUsd: 0.01, perSeedMaxBytes: 10, renderEgressUnits: 8 });
...
test("BudgetTracker: transformReserved false (raw seed) records bytes but no cost + no re-check", () => {
  const clock = fakeClock();
  const t = new BudgetTracker({ clock, maxGlobalEgressBytes: 1000, maxGlobalWallMs: 5000, maxTransformCostUsd: 0.001, perSeedTransformCostUsd: 0.01, perSeedMaxBytes: 10, renderEgressUnits: 8 });
  const b = t.beforeSeed();
...
test("BudgetTracker: a $0 cost cap (perSeed clamps to 0) skips the transform — no paid call slips through", () => {
  ...
  const clock = fakeClock();
  const t = new BudgetTracker({ clock, maxGlobalEgressBytes: 1_000_000, maxGlobalWallMs: 5000, maxTransformCostUsd: 0, perSeedTransformCostUsd: 0, perSeedMaxBytes: 100, renderEgressUnits: 8 });
  const b = t.beforeSeed();
...
test("BudgetTracker: cancelReservation releases a dispatch reservation without recording settled bytes", () => {
  const clock = fakeClock();
  const t = new BudgetTracker({ clock, maxGlobalEgressBytes: 100, maxGlobalWallMs: 5000, maxTransformCostUsd: 1, perSeedTransformCostUsd: 0.02, perSeedMaxBytes: 60, renderEgressUnits: 8 });
...
test("BudgetTracker: reserveRetry reserves a 2nd byte unit but skips when it would breach the cap (codex P2)", () => {
  ...
  const t2 = new BudgetTracker({ clock, maxGlobalEgressBytes: 200, maxGlobalWallMs: 5000, maxTransformCostUsd: 1, perSeedTransformCostUsd: 0.02, perSeedMaxBytes: 60, renderEgressUnits: 8 });
...
test("BudgetTracker: reserveRender reserves opts.renderEgressUnits × perSeed (caller-computed, decoupled from maxBytes #143) and skips when it would breach the cap (codex R5/R7/R11)", () => {
  const clock = fakeClock();
  // renderEgressUnits is now CALLER-COMPUTED (was a hardcoded 8×): the essential pool was decoupled
  // from maxBytes (#143, fixed 48MB), so the render reservation is per-call. reserveRender must honor
  // whatever the caller passes. perSeed 10, renderEgressUnits 5: 1 unit (10) + 5×10=50 → 60 ≤ 100 fits.
  const t = new BudgetTracker({ clock, maxGlobalEgressBytes: 100, maxGlobalWallMs: 5000, maxTransformCostUsd: 1, perSeedTransformCostUsd: 0.02, perSeedMaxBytes: 10, renderEgressUnits: 5 });
  const b = t.beforeSeed();
  assert.equal(b.dispatch, true);
  assert.equal(t.reserveRender(), true, "1 unit (10) + render pool (5×10=50) = 60 ≤ 100 → render allowed");
  const after = t.afterSeed({ bytes: 45, transformReserved: b.runTransform, byteUnits: 1 + 5 });
  assert.equal(after.shortCircuit, false, "45 ≤ 100 → no breach");

  // Same perSeed/cap but renderEgressUnits 10 (heavier render): 1 unit (10) + 10×10=100 → 110 > 100 → refuse.
  const t2 = new BudgetTracker({ clock, maxGlobalEgressBytes: 100, maxGlobalWallMs: 5000, maxTransformCostUsd: 1, perSeedTransformCostUsd: 0.02, perSeedMaxBytes: 10, renderEgressUnits: 10 });
  const b2 = t2.beforeSeed();
  assert.equal(b2.dispatch, true);
  assert.equal(t2.reserveRender(), false, "1 unit (10) + render pool (10×10=100) = 110 > 100 → render refused");
